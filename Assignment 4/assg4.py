...
# ===============================
def align_images(
        image_to_align_path, reference_image_path,
    ...
    if des1 is None or des2 is None or len(kp1) < 4 or len(kp2) < 4:
        raise RuntimeError(f"{method.upper()} failed: not enough keypoints/descriptors found.")
    ...
    if H is None:
        raise RuntimeError(f"{method.upper()} failed: could not compute homography.")
...
if __name__ == "__main__":
    ...
    # ⿢ Feature Alignment - SIFT, SURF, ORB
    methods = [
        ('sift', 10, 0.7),
        ('surf', 10, 0.7),
        ('orb', 1500, 0.15)
    ...
    for m, f, g in methods:
        try:
            ...
            align_images(
                align_img, ref_img,
                method=m, max_features=f, good_match_percent=g,
